...
RELEASE="stable"
ERROR_LOG="/opt/opennms_config_stage/logs/bootstrap.log"
DB_USER="opennms"
DB_PASS="opennms"
...
initializePostgres() {
  ...
  /usr/pgsql-15/bin/postgresql-15-setup initdb 1>>"${ERROR_LOG}" 2>>"${ERROR_LOG}"
  checkError "${?}"
  echo -n "PostgreSQL set auth from ident to scram-sha-256 ... "
  sed -i 's/all             127\.0\.0\.1\/32            ident/all             127.0.0.1\/32            /g' /var/lib/pgsql/15/data/pg_hba.conf
  sed -i 's/all             ::1\/128                 ident/all             ::1\/128                 scram-sha-256/g' /var/lib/pgsql/15/data/pg_hba.conf
  checkError "${?}"
  echo -n "Start PostgreSQL database             ... "
  systemctl start postgresql-15
  checkError "${?}"
  echo -n "PostgreSQL systemd enable             ... "
  systemctl enable postgresql-15 1>>"${ERROR_LOG}" 2>>"${ERROR_LOG}"
  checkError "${?}"
}
