##############################################################################
##
##  Oracle_blocking_locks.sh - Oracle DB blocking locks OpenNMS  Monitoring 
##
...
##
##  eg.
##    Oracle_blocking_locks.sh server_DB abbmon pass DB
##
##
##  Dependencies:-
##
...
# If it is run manually ensure the full path is used and a SID is passed
   ...
   if [ "$SCRIPT" = "Oracle_blocking_locks.sh" ] || [ "$SCRIPT" = "./Oracle_blocking_locks.sh" ] ; then
...
select 'Blocking Lock= '||c.pname||' '||s1.username||'@'||s1.machine||' ('||s1.sid||','||s1.serial#||') blocking 
