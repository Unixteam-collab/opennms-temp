...
die "Wrong arguements: check_ellipsebatch_longruni_new.pl dbSid nodeName tableName dbuserid dbpasswd" if ($#ARGV != 4);
...
select -- List of values to display with alert
       prog_name, dstrct_code, Start_Date, Start_Time, Runtime_Minutes, Median_Elapsed, Std_Deviation, Count_Instances, UUID
from
(
...
where Runtime_Minutes > (4*Std_Deviation+Median_Elapsed + 5)
order by 1,2,3,4,5
END
...
while ( my ($prog_name, $uuid, $district_code, $runtime_minutes, $priorCount, $threshold) = $arraySelect->fetchrow_array() ) {
    ...
    if ( $runtime_minutes+0 > $threshold ) {
        ...
        print "$errorMsg\n";
     	system "echo send-event.pl uei.opennms.org/ABBCS/Batch/failure-$errSev -n $targetDeviceId -d \"Batch failure\" -p \"label Batch\" -p \"resourceId $prog_name:$uuid\" -p \"ds batchFailure\" -p \"description $errorMsg.  To change threshold, add/update entry in `hostname`:$thresholdFile  Format PROGNAME,THRESHOLD,PRIORITY\" -p \"uuid $uuid\"";
